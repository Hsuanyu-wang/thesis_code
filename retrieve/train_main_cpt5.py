...
# CWQ optimized path (batched/advanced data pipeline)
try:
...
###################################################################//
# CWQ-specific optimized helpers and train/eval (batched)
###################################################################\\
...
@torch.no_grad()
def eval_epoch_cwq(config, device, data_loader, model, args=None, epoch=None, num_epochs=None):
    model.eval()
    metric_dict = defaultdict(list)
    val_losses = []
    desc = f"Validation"
    ...
    for batch in eval_pbar:
        ...
        for i in range(len(pred_triple_logits_batch)):
            ...
            if 'target_triple_probs_list' in batch:
                target = batch['target_triple_probs_list'][i]
                pos_weight_factors = batch['pos_weight_factors_list'][i] if 'pos_weight_factors_list' in batch else None
            else:
                target = batch['target_triple_probs'][i]
                pos_weight_factors = None
            ...
            if target.numel() != pred_triple_logits.numel():
                ...
                if pos_weight_factors is not None:
                    pos_weight_factors = pos_weight_factors[:min_len]

            # 根據不同的權重方法計算驗證 loss
            if args and args.spcount:
                # spcount: 使用 shortest path 計數作為權重
                if pos_weight_factors is not None:
                    sp_weights = pos_weight_factors.clone()
                    positive_mask = (target > 0).float()
                    bce_loss = F.binary_cross_entropy_with_logits(
                        pred_triple_logits, positive_mask, reduction='none')
                    weighted_loss = bce_loss * sp_weights
                    val_loss = weighted_loss.mean()
                else:
                    # 回退到標準權重
            ...
            elif args and args.spcount_inv:
                # spcount_inv: 使用 shortest path 計數的倒數作為權重
                if pos_weight_factors is not None:
                    sp_weights = pos_weight_factors.clone()
                    positive_mask = (target > 0).float()
            ...
            # Metrics
            sorted_ids = torch.argsort(pred_triple_logits, descending=True).cpu()
            triple_ranks_pred = torch.empty_like(sorted_ids)
            triple_ranks_pred[sorted_ids] = torch.arange(len(triple_ranks_pred))
            target_triple_ids = (target > 0).nonzero().squeeze(-1).cpu()
            num_target_triples = len(target_triple_ids)
            if num_target_triples == 0:
                continue
    ...
    metric_dict['val_loss'] = np.mean(val_losses) if val_losses else float('inf')
    return metric_dict


def train_epoch_cwq(device, train_loader, model, optimizer, args=None, epoch=None, num_epochs=None):
    model.train()
    total_loss = 0.0
    num_samples = 0
    desc = f"Training"
    ...
    for batch in train_pbar:
        ...
        valid = 0
        for i in range(len(pred_triple_logits_batch)):
            pred_logits = pred_triple_logits_batch[i].reshape(-1)
            if 'target_triple_probs_list' in batch:
                target = batch['target_triple_probs_list'][i]
                pos_weight_factors = batch['pos_weight_factors_list'][i] if 'pos_weight_factors_list' in batch else None
            else:
            ...
            if target.numel() != pred_triple_logits_batch[i].numel():
...
def main(args):
    ...
    if use_cwq_optimized:
        ...
        # Choose batch size (prefer config if available)
        batch_size = 64
        try:
            batch_size = int(config['train'].get('batch_size', batch_size))
        except Exception:
            pass
        # Use grouped sampler if available on dataset for I/O locality
        if hasattr(train_set, 'emb_dict') and hasattr(train_set.emb_dict, '_sample_to_batch'):
        ...
        if hasattr(val_set, 'emb_dict') and hasattr(val_set.emb_dict, '_sample_to_batch'):
    ...
    for epoch in main_pbar:
        ...
        # Training phase
        if use_cwq_optimized:
            train_log_dict = train_epoch_cwq(device, train_loader, model, optimizer, args, epoch, num_epochs)
        else:
            train_log_dict = train_epoch(device, train_loader, model, optimizer, args, epoch, num_epochs)
        
        # Validation phase
        if use_cwq_optimized:
            val_eval_dict = eval_epoch_cwq(config, device, val_loader, model, args, epoch, num_epochs)
        else:
