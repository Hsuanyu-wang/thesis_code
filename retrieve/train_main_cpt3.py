###################################################################//
# cpt3
# + kwm(score, score_inv, prob, prob_inv form to weight BCE loss)
# + kgf(effect shortest path forming, optimal subgraph triplet sets)
# + record eval kge_model
###################################################################\\
import numpy as np
import os
import pandas as pd
...
def main(args):
    ...
    print(f"   KGE freq weight: {args.kge_freq_weight}")
    if args.kge_weight or args.kge_freq_weight:
    ...
    if args.kge_freq_weight:
        feature_tags.append(f'kge_freq_{args.kge_model}_{args.kge_weight_mode}')
    if args.use_dropout:
    ...
    train_set = RetrieverDataset(
        ...
        kge_freq_weight=args.kge_freq_weight, kge_scorer=kge_scorer
    )
    ...
    for epoch in main_pbar:
        ...
        if metric_improved or loss_improved:
            ...
            with open(info_path, 'w') as f:
                ...
                f.write(f"  KGE freq weight: {args.kge_freq_weight}\n")
                if args.kge_weight or args.kge_freq_weight:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-kw', '--kge_weight', action='store_true', help='Enable KGE model-based weighting.')
    parser.add_argument('-kgf', '--kge_freq_weight', action='store_true', help='Enable KGE-based score weighting (affects shortest path triplet sets).')
    parser.add_argument('-km', '--kge_model', type=str, default='transe', 
                       choices=['transe', 'distmult', 'complex', 'auto'], 
