...
# AMP disabled for baseline reproduction
...
# --- Minimal helper to avoid NameError and print basic environment info ---
def check_and_warn_resources(args):
    try:
        ...
        if not torch.cuda.is_available() and getattr(args, 'batch_size', 64) > 64:
            print("⚠️ Consider reducing --batch_size when training on CPU.")
...
def test_pipeline(args):
    ...
    try:
        ...
        for j in range(len(pred_triple_logits_batch)):
            ...
            if getattr(args, 'method_sp', 'none') in ['spcount', 'spcount_inv']:
                ...
                loss += F.binary_cross_entropy_with_logits(pred_logits, target)
...
def main(args):
    ...
    if use_legacy_mode:
        ...
        # 記錄triplet統計信息 (Legacy 模式)
        triplet_info = {
            'train': {
                'skipped_samples': getattr(train_set, 'num_skipped', 0),
                'relevant_triples_median': getattr(train_set, 'median_num_relevant', 0),
                'relevant_triples_mean': getattr(train_set, 'mean_num_relevant', 0),
                'relevant_triples_max': getattr(train_set, 'max_num_relevant', 0)
            },
            'val': {
                'skipped_samples': getattr(val_set, 'num_skipped', 0),
                'relevant_triples_median': getattr(val_set, 'median_num_relevant', 0),
                'relevant_triples_mean': getattr(val_set, 'mean_num_relevant', 0),
                'relevant_triples_max': getattr(val_set, 'max_num_relevant', 0)
            }
        }
    else:
    ...
    for epoch in main_pbar:
        ...
        if use_legacy_mode:
            ...
            args.grad_accum_steps, epoch, num_epochs, use_pos_weight=(method_sp in ['spcount', 'spcount_inv'])
        )
        ...
        if use_legacy_mode:
            ...
            config, device, val_loader, model, epoch, num_epochs, use_pos_weight=(method_sp in ['spcount', 'spcount_inv'])
        )
