# 這是安全版 prepare_data_safe.py，修正 pred_file_path 不存在或為空時不會 open 檔案
import os
import re
import json
import torch
...
def add_good_triplets_from_rog(data):
    ...
    for idx, each_qa in enumerate(tqdm(data)):
        paths_str = extract_reasoning_paths(each_qa["input"])
...
def get_data(dataset_name, pred_file_path, score_dict_path, split, prompt_mode, seed=0, triplets_to_sample=[50, 100, 200, 300]):
    ...
    if pred_file_path and os.path.exists(pred_file_path):
        ...
        # fallback: load from original source
        print("Prediction file not found, loading from original dataset...")
        input_file = os.path.join("rmanluo", f"RoG-{dataset_name}")
        subgraphs = load_dataset(input_file, split=split)
        raw_data = []
        for i, each_qa in enumerate(tqdm(subgraphs)):
            each_qa = dict(each_qa)  # 保證是 dict
            each_qa["id"] = each_qa.get("id", str(i))
            # 嘗試取得 input 欄位，沒有就用 question 或空字串
            if "input" in each_qa:
                each_qa["input"] = each_qa["input"]
            elif "question" in each_qa:
                each_qa["input"] = each_qa["question"]
            else:
                each_qa["input"] = ""
            raw_data.append(each_qa)
